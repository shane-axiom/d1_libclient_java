...
package org.dataone.client.v2.types;
...
//    	sm.setAuthoritativeMemberNode(nodeId);
	...
	public static SystemMetadata cloneSystemMetadata(SystemMetadata sm) 
	{
        ...
        clone.setSeriesId(D1TypeBuilder.cloneIdentifier(sm.getSeriesId()));
        clone.setFileName(sm.getFileName());
        if (sm.getMediaType() != null) {
            clone.setMediaType(new MediaType());
            clone.getMediaType().setName(sm.getMediaType().getName());
            if (sm.getMediaType().sizePropertyList() > 0) {
                List<MediaTypeProperty> cloneMTProps = new LinkedList<MediaTypeProperty>();
                for (MediaTypeProperty mtp : sm.getMediaType().getPropertyList()) {
                    cloneMTProps.add(D1TypeBuilder.cloneMediaTypeProperty(mtp));
                }
                clone.getMediaType().setPropertyList(cloneMTProps);
            }
        }
        return clone;
	}
	...
	public static MediaTypeProperty cloneMediaTypeProperty(MediaTypeProperty mtp) {
	    if (mtp == null) return null;
	    
	    MediaTypeProperty clone = new MediaTypeProperty();
	    if (mtp.getName() != null)
	        clone.setName(mtp.getName());
	    
	    if (mtp.getValue() != null) 
	        clone.setValue(mtp.getValue());
