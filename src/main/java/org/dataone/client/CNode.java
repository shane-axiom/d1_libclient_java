...
public class CNode extends D1Node 
  implements CoordinatingNodeCrud, CoordinatingNodeAuthorization, 
  CoordinatingNodeAuthentication {
...
//      // TODO: also set the system metadata to the same perms
  ...
  /**
   * Deserialize an input stream to an ObjectFormatList
   * 
   * @param is - the input stream to deserialize
   * @return objectFormatList - the object format list
   * @throws JiBXException
   */
  protected ObjectFormatList deserializeObjectFormatList(InputStream is)
    throws JiBXException {
  ...
  @Override
  public boolean setAccess(AuthToken token, Identifier pid, 
   ...
   * access its services by looing up the registration for the node at 
  ...
  /**
   * Initialize the map of nodes (paids of NodeId/NodeUrl) by getting the 
   * map from the CN and parsing the XML, storing the node information in 
   * the nodeMap map. These values are used later to look up a node's URL 
   * based on its ID, or its ID based on its URL.
   * @throws IOException
  ...
  /**
   * List the object formats registered with the Coordinating Node. Object
   * formats should be synchronized across CNs, and so results should be the
   * the same from any given CN.
   * 
   * @author cjones
  ...
  public ObjectFormatList listFormats() 
    ...
    // create the D1 URL ("http://server/service/formats")
    ...
    try {
      // get the ObjectFormatList type from the input stream
      objectFormatList = deserializeObjectFormatList(is);
